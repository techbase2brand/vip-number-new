...
import { BASE_DIGITAL_CARD_PLANS, ADD_ON_OPTIONS, ALL_IN_ONE_ADD_ON_AMOUNT } from "./planOptions";
...
const Contectless = () => {
  ...
  const calculatePricing = (cardId, basePlanId) => {
    const basePlan = BASE_DIGITAL_CARD_PLANS.find(plan => plan.id === basePlanId);
    ...
    if (card.planConfig.addOns.smart && card.planConfig.addOns.stand) {
      ...
      // Only smart card
      addOnPrice = ADD_ON_OPTIONS.find(opt => opt.id === 'smart')?.amount ?? 499;
    } else if (card.planConfig.addOns.stand) {
      // Only stand
      addOnPrice = ADD_ON_OPTIONS.find(opt => opt.id === 'stand')?.amount ?? 999;
    }
                ...
                <div className="bg-white rounded-b-3xl text-center text-gray-800 py-4 md:px-4 w-full flex flex-col items-center h-[58%] justify-between">
                  ...
                  <div className="w-full mb-3">
                    ...
                    <select
                      ...
                      onChange={(e) => handlePlanChange(item.id, e.target.value)}
                    >
                  ...
                  {/* Pricing Breakdown */}
                  {(() => {
                    const pricing = calculatePricing(item.id, selectedPlans[item.id]);
                    const basePlan = BASE_DIGITAL_CARD_PLANS.find(p => p.id === selectedPlans[item.id]);
                        ...
                        {item.planConfig.addOns.smart && item.planConfig.addOns.stand ? (
                          ...
                          <>
                            ...
                            <div className="text-xs text-green-600 italic">
                              Save ₹{(smartAmount + standAmount) - pricing.addOnPrice}
                            </div>
                          </>
